...
<?php
	...
	$idpaciente = get_param("idpaciente");
			...
			$Sql = "SELECT to_char(tc.Fecha,'YYYY-MM-DD HH24:MI:SS'), tp.Nombre, te.Nombre, decode(Estado_turno,'A','Asignado','S','Sin asignar','C','Cancelado','O','Asistio','Otro') Estado
				FROM T_HCP_CALENDARIO_TURNO tc, T_HCP_PROFESIONAL_SALUD tp, T_HCP_ESPECIALIDAD te
				WHERE ( (tc.Profesional_salud = tp.Identificacion) 
						and (tp.Especialidad = te.Codigo) 
						and (tc.Paciente=".$idpaciente.") 
					) ORDER BY Fecha DESC";		
	...
	else 
	{
		$rs->MoveFirst();
?>
<form name="rsv_ver_citas" method="post" action="rsv_register_reserva.php"  onsubmit="return validar('rsv_ver_citas');">
	<h3>Listado de reservas hechas </h3>
  <table width="80%" border="0" bgcolor="#666666" >
    <tr bgcolor="#999999" > 
      <th>Fecha </th>
      <th>Doctor </th>
      <th>Especialidad </th>
